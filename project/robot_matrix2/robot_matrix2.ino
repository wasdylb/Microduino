...
void loop() {
  ...
  clean(1000);
    ...
    vosetlcd("Hello Microduino!", -6 * 17 , LED_ON, 150);
    vosetimg(smile1_bmp, LED_ON, 100);
  */

  for (int x = 0; x <= 8; x++)
  {
    for (int j = 0; j <= 8; j++)
      vosetimge(love2_bmp, LED_ON, 50, 0, 0, x, 8 );
  }
  for (int i = 0; i < 6; i++)
  {
    vosetimg(love2_bmp, LED_ON, 800);
    clean(500);
  }
  ...
  {
    for (int j = 0; j <= 8; j++)
      vosetimge(love1_bmp, LED_ON, 50, 0, 0, x, 8 );
  }
  for (int i = 0; i < 6; i++)
  {
    vosetimg(love1_bmp, LED_ON, 800);
    clean(500);
  }
  /*
    for (int i = 0; i < 3; i++)
    {
      vosetimg(smile1_bmp, LED_ON, 100);
      vosetimg(smile2_bmp, LED_ON, 100);
    }
    */
}
...
void logo_move()
{
  for (int i = 0; i < 8; i++)
  {
    ...
    matrix.drawBitmap(tempJ, tempI, logo_bmp, 8, 8, LED_ON);
    matrix.writeDisplay();
    if (tempI + 4 == 8) {
      left = true;
    }
